...
path_ts = "C:...\\stream.ts"# put here the path and the name of the ".ts" initial video (in the complete path include ".ts" at the end)
path_mp4 = "C:...\\stream.mp4"# put here the path and the name of the file  ".mp4" final video  (in the complete path include ".mp4" at the end)
...
async def download_chunk(url,session, vod_id, chunk):
    ...
    while True:
        try:
            ...
            print(f"Error downloading chunk {chunk}: {e}  status: {resp.status}")  # if it fails fetching is gonna try again until it fetched
...
async def main():
    async with aiohttp.ClientSession() as session:
        ...
        while True:
            try: 
                async with session.get(link,headers=headers) as resp:
                    ...
                    m3u8_url = f"https://{dom}.cloudfront.net/{vod_id}/chunked/index-dvr.m3u8" # forgin mru8 url
                    url = f"https://{dom}.cloudfront.net/"
                    print("URL:", m3u8_url)
                    break
            except: continue
        ...
        tasks = [download_chunk(url,session, vod_id, chunk) for chunk in chunks]
